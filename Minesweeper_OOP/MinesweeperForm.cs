...
namespace ProfessionalMinesweeper
{
    public partial class MinesweeperForm : Form
    {
        ...
        private void InitializeGame(DifficultyLevel difficulty)
        {
            ...
            timeElapsed = 0;
            tsslTimeElapsed.Text = "Zaman: 0";
            tsslMinesLeft.Text = $"Mayınlar: {gameBoard.MineCount}";
        ...
        private void ToggleFlag(Cell cell, Button button)
        {
            if (!cell.IsRevealed)
            {
                ...
                {
                    button.Text = "🚩";
                    if (cell.IsMine) gameBoard.FlaggedMineCount++;
                }
                else
                {
                    button.Text = "";
        ...
        private void customToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (var customDialog = new CustomDifficultyDialog())
            {
                ...
                {
                    gameBoard = new Board(customDialog.Width, customDialog.Height, customDialog.Mines);
